...
class DepthMaskGenerator:
    """Generates foreground masks using DepthAnything V2 for masked loss computation."""
    
    def __init__(self, encoder='vitb', percentile=50, mask_kernel=(21, 21), mask_sigma=10, device='cuda'):
        """
        Args:
    ...
    def generate_masks(self, img_tensor):
        """
        ...
        Returns:
            Mask tensor of shape (B, C, H, W) in range [0, 1] float32
            where 1 = foreground (keep for loss), 0 = background (ignore for loss)
        """
        ...
        for i in range(batch_size):
            ...
            for f in range(frames):
                ...
                mask_3ch = np.stack([mask, mask, mask], axis=0)  # (3, H, W)
                frame_masks.append(mask_3ch)
            
            # Combine masks for all frames: (frames*3, H, W)
            combined_mask = np.concatenate(frame_masks, axis=0)
            mask_batch.append(torch.from_numpy(combined_mask))
        
        # Stack to (B, C, H, W) and move to device
        return torch.stack(mask_batch).to(img_tensor.device).float()
...
def masked_mse_loss(pred, target, mask):
    """
    Compute MSE loss only on masked (foreground) pixels.
    
    Args:
        pred: Predicted tensor (B, C, H, W)
        target: Target tensor (B, C, H, W)
        mask: Binary mask (B, C, H, W) where 1 = compute loss, 0 = ignore
    ...
    # Compute mean over masked pixels only
    # Sum over all dimensions and divide by number of masked elements
    num_masked_elements = mask.sum()
    if num_masked_elements > 0:
        loss = masked_error.sum() / num_masked_elements
    else:
        # Fallback to regular MSE if mask is empty (shouldn't happen)
...
def train_lapo(config: LAPOConfig):
    ...
    for epoch in trange(config.num_epochs, desc="Epochs"):
        ...
        for batch in dataloader:
            ...
            # update lapo with masked loss
            with torch.autocast(DEVICE, dtype=torch.bfloat16):
                pred_next_obs, latent_action = lapo(obs, future_obs)
                # Use masked MSE loss instead of regular MSE
                loss = masked_mse_loss(pred_next_obs, next_obs, next_obs_mask)
...
def train(config: Config):
    ...
    # stage 1: pretraining lapo on unlabeled dataset with masked loss
